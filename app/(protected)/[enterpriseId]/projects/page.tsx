...
export default function ProjectsPage() {
  ...
  useEffect(() => {
    fetchProjects();
  }, [searchTerm, statusFilter, priorityFilter, enterpriseId]);
  ...
  const fetchProjects = async () => {
    try {
      ...
      if (priorityFilter) queryParams.append('prioridade', priorityFilter);
      queryParams.append('enterpriseId', enterpriseId.toString());
          ...
          <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6'>
            {projects.map((project) => (
              <div
                key={project.id}
                onClick={() =>
                  router.push(`/${enterpriseId}/projects/${project.id}`)
                }
                onKeyDown={(e) => {
