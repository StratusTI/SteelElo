...
export async function DELETE(_req: Request, { params }: RouteParams) {
  ...
  try {
    ...
    const existingIntegration = await integrationsRepository.findByUserAndProvider(
      user.id,
      provider
    )

    if (existingIntegration) {
      await revokeToken(provider as OAuthProvider, existingIntegration.accessToken)
    }
    ...
    return successResponse({ provider }, 200, 'Integracao desconectada com sucesso')
  } catch (err) {
    ...
    return standardError('INTERNAL_SERVER_ERROR', 'Erro ao desconectar integracao')
  }
}
