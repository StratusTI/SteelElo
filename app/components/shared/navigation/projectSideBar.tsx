...
import { AiMagicIcon, GithubIcon, SlackIcon } from '@hugeicons-pro/core-bulk-rounded';
import {
  Add01Icon,
  Analytics01Icon,
  ArrowRight01Icon,
...
export function SideBarProjects() {
  ...
  const pageTree: PageTreeItem[] = [
    ...
    { name: 'your-work', label: 'Seu trabalho', path: '/projects/your-work', icon: UserListIcon },
    { name: 'your-projects', label: 'Projetos', path: '/projects', icon: WorkIcon },
    { name: 'stickies', label: 'Post-its', path: '/projects/stickies', icon: NoteIcon },

    {
      ...
      defaultOpen: true,
      items: [
        { name: 'backlog', label: 'Backlog', path: '/projects/backlog', icon: DashedLineCircleIcon },
        { name: 'sprints', label: 'Ciclos', path: '/projects/sprints', icon: RepeatIcon },
        { name: 'dependencies', label: 'Dependências', path: '/projects/dependencies', icon: UserLock02Icon },
        { name: 'analytics', label: 'Análises', path: '/projects/analytics', icon: Analytics01Icon },
        { name: 'dashboards', label: 'Painéis de controle', path: '/projects/dashboards', icon: KanbanIcon },
        { name: 'daily-standup', label: 'Daily Standup', path: '/projects/daily-standup', icon: ComputerUserIcon },
      ]
    },
...

    {
      ...
      defaultOpen: true,
      items: [
        { name: 'github', label: 'Conectar ao GitHub', path: '/settings/github', icon: GithubIcon },
        { name: 'slack', label: 'Conectar ao Slack', path: '/settings/slack', icon: SlackIcon },
        { name: 'ai', label: 'Testar Elo AI', path: 'settings/ai', icon: AiMagicIcon },
      ]
    }
  ];
          ...
          <CollapsibleContent>
            <div>
              {projects.map((project) => {
                const iconData = project.icone as [string, { [key: string]: string | number }][] | undefined;
                const hasValidIcon = iconData && Array.isArray(iconData) && iconData.length > 0;
                ...
                return (
                  <Button
                    ...
                    onClick={() => router.push(buildPath(`/project/${project.id}`))}
                  >
  ...
  return (
    <>
      <div className='flex flex-col gap-3 pb-3'>
        ...
        >
          <Icon icon={KeyframesMultipleAddIcon} strokeWidth={2} className='rotate-218'/> Novo item de
          trabalho
        </Button>
      </div>
