...
import { createFakeUser, createFakeUserDTO } from '@/src/__tests__/factories/user.factory'
import { expectOk, expectErr } from '@/src/__tests__/helpers/result.helpers'
...
describe('UserService', () => {
  describe('getProfile()', () => {
    ...
    it('should fetch from repository and populate cache on cache miss', async () => {
      ...
      expect(mockedCache.set).toHaveBeenCalledWith('user-1', expect.objectContaining({ id: 'user-1' }))
    })
  ...
  describe('updateProfile()', () => {
    it('should update name successfully and invalidate cache', async () => {
      ...
      expect(mockedRepo.update).toHaveBeenCalledWith('user-1', { name: 'New Name' })
    ...
    it('should return conflict when email belongs to another user', async () => {
      const existingUser = createFakeUser({ id: 'other-user', email: 'taken@example.com' })
      mockedRepo.findByEmail.mockResolvedValue(ok(existingUser))

      const result = await UserService.updateProfile('user-1', { email: 'taken@example.com' })
    ...
    it('should allow updating to own current email', async () => {
      const currentUser = createFakeUser({ id: 'user-1', email: 'my@example.com' })
      mockedRepo.findByEmail.mockResolvedValue(ok(currentUser))
      mockedRepo.update.mockResolvedValue(ok(currentUser))
      mockedCache.invalidate.mockResolvedValue(undefined)

      const result = await UserService.updateProfile('user-1', { email: 'my@example.com' })
    ...
    it('should allow email update when email is not taken', async () => {
      const updatedUser = createFakeUser({ id: 'user-1', email: 'new@example.com' })
      mockedRepo.findByEmail.mockResolvedValue(ok(null))
      mockedRepo.update.mockResolvedValue(ok(updatedUser))
      mockedCache.invalidate.mockResolvedValue(undefined)

      const result = await UserService.updateProfile('user-1', { email: 'new@example.com' })
    ...
    it('should propagate findByEmail repository error', async () => {
      ...
      const result = await UserService.updateProfile('user-1', { email: 'any@example.com' })
